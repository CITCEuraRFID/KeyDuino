...
bool EmulateTag::emulate(const uint16_t tgInitAsTargetTimeout){
  ...
  if(1 != keyduino.tgInitAsTarget(command,sizeof(command), tgInitAsTargetTimeout)){
  ...
  while(runLoop){
    status = keyduino.tgGetData(rwbuf, sizeof(rwbuf));
    if(status < 0){
    ...
    status = keyduino.tgSetData(rwbuf, sendlen);
    if(status < 0){
